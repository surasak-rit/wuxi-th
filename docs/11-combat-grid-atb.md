...
> **โหมดต่อสู้ทางเลือก (Real-time Action)** — ต่างจาก `docs/01` (turn-based text log)
> ทุกหน่วยมี **หลอดคูลดาวน์ (Action Gauge)** อิสระที่เติมตามเวลาจริง, เคลื่อนที่บนกริด
> พร้อมกันทุกตัวโดยไม่สลับเทิร์น, เมื่อหลอดเต็มใช้สกิล **AoE รอบตัว (Radial)** ใส่ฝูงศัตรู
> และแสดง **Floating Damage Text** เหนือหัวศัตรูทุกตัวพร้อมกัน
>
> ⚠️ **หมายเหตุการ reconcile:** เกมหลักเดิมเป็น text-based turn-based — เอกสารนี้ออกแบบให้
> **ใช้ค่าสถานะ/สูตรดาเมจชุดเดียวกับ docs/01 §3–4** เพื่อให้สลับเป็นระบบหลักหรืออยู่เป็น
> โหมดเสริม (เช่น โหมดล้างฝูงมอน/raid) ได้โดยไม่ต้องรื้อ progression/วิชา/ไอเทม
...
```
> **คำแนะนำ:** ถ้าเน้น **Radial AoE รอบตัว** เป็นจุดขาย → **hex** ให้รัศมีสมมาตรสวยและ
> ยุติธรรมทุกทิศ; ถ้าเน้นความเร็วในการสร้างคอนเทนต์/terrain เมือง → **square (8 ทิศ)**
> เอนจินออกแบบให้สลับ mode ได้ผ่าน config โดย logic การเลือกเป้าอ้าง `grid_dist()` ตัวเดียว
...
- [ ] เลือกโทโพโลยีกริดเป็น **สี่เหลี่ยม หรือ หกเหลี่ยม** (มีผลต่อ art/UI/คณิตระยะ — §3)
- [ ] โหมดนี้ **แทน** turn-based (docs/01) เป็นระบบหลัก หรืออยู่เป็น **โหมดเสริม** (ล้างฝูง/raid)?
